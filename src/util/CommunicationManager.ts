...
export default class CommunicationManager {
  ...
  async loadProfile(webidprofile: string): Promise < N3.N3Store | null > {
  ...
  async getDataStoreFromFile(fileURI: string): Promise < N3.N3Store > {
  ...
  async getFullNameFromProfile(profileId: string): Promise < string > {
  ...
  async getFullNameFromStore(store: N3.N3Store): Promise < string > {
    let names = [];
    ...
    if (names.length === 0)
      names = await store.getQuads(null, FOAF + "name", null, null);
  ...
  async initializeResearchPaperStorage(
    ...
    let collection = await this.getResearchPaperCollectionFromStore(store);
    if (!papersDirectoryURI) {
      papersDirectoryURI = this.getBaseIRI(profileURI) + "papers/";
    }
    if (!collection) {
      if (!papersDirectoryURI.endsWith("/"))
        papersDirectoryURI = papersDirectoryURI + "/";
      this.patchProfileWithPaperCollection(profileURI, papersDirectoryURI);
    }
    let newProfileStore = await this.getDataStoreFromFile(profileURI);
  ...
  async getResearchPaperCollectionFromFile(
    ...
    collectionid: string;viewid ? : string | null
  } | null > {
  ...
  async getResearchPaperCollectionFromStore(
    ...
    for (let id of collectionIds) {
      const quads = store.getQuads(id, HYDRA + "view", null, null);
      if (quads.length > 0) {
        return {
          collectionid: id,
  ...
  async getResearchPapers(profileURI: string): Promise < Array < PaperMetadata >> {
  ...
  async getResearchPapersFromFile(fileURI: string): Promise < Array < PaperMetadata >> {
    ...
    return paperIds.map((paperId: string) => {
      ...
      return {
        id: paperId,
        title: titleQuad && (titleQuad.object.value || titleQuad.object.id),
        metadatalocation: locationQuad && (locationQuad.object.value || locationQuad.object.id),
        publisher: publisherQuad && (publisherQuad.object.value || publisherQuad.object.id)
      };
    });
  }
  ...
  async patchProfileWithPaperCollection(
    ...
    papersDirectoryURI = papersDirectoryURI || "/" + DEFAULTPAPERSDIRECTORY;
    const profileURIhashtag = profileURI.split("#")[0] + "#";
    const paperCollectionURI = profileURIhashtag + PARERSCOLLECTIONNAME;
    const partialCollectionViewId = papersDirectoryURI + PAPERSCOLLECTIONFILE;
    const contents =
      "INSERT DATA {  \
        <" + paperCollectionURI + "> <" + RDF + "type> <" + HYDRA + "Collection> ; \n \
      <" + DCTERMS + 'description> "Collection of research papers" ; \n \
      <' + DCTERMS + "subject> <" + RESEARCH_PAPER_CLASS + "> ; \n \
      <" + HYDRA + "view> <" + partialCollectionViewId + "> . \
      <" + partialCollectionViewId + "> <" + RDF + "type> <" + HYDRA + "PartialCollectionView> }";
  ...
  async addPaperCollectionFile(
    ...
    papersDirectoryURI =
      (papersDirectoryURI || DEFAULTPAPERSDIRECTORY) + PAPERSCOLLECTIONFILE;
  ...
  async addPaper(
    ...
    const store = await this.getDataStoreFromFile(metadata.publisher);
    const collection = await this.getResearchPaperCollectionFromStore(store);
    if (!collection)
      throw new Error("User not logged in or could not access user profile");

    // Upload the file to the solid pod.
    const paperURI = metadata.id || DEFAULTPAPERSDIRECTORY + PAPERSCOLLECTIONFILE;
    ...
    if (fileUploadResponse.status === 201) {
      ...
      metadataURI = (metadataURI.slice(0, Math.max(1, (metadataURI.length - 1))).join(".")) + "_meta.ttl"
      ...
      await this.fu.patchFile(collection.viewid || collection.collectionid, payload);

      await this.createMetadataFile(uploadURL, metadataURI, metadata)
    } else {
  ...
  async createMetadataFile(paperURI: string, metadataURI: string, metadata: PaperMetadata) {
  ...
  async addComment(commentMetadata: CommentMetadata, paperMetadata: PaperMetadata, inboxes ? : string[]) {
    const directory = commentMetadata.commentLocation || DEFAULTCOMMENTSDIRECTORY
    const extension = ".ttl"
    const fileName = await this.getRandomizedFileName(directory, extension, "comment")
    const commentURI = directory + fileName + extension;
    const comment = MetadataFileGenerator.createComment(commentURI, commentMetadata.documentId, commentMetadata.publisherId, commentMetadata.text)

    await this.fu.postFile(commentURI, comment.payload, "text/turtle")

    const metadataLocation = paperMetadata.metadatalocation || paperMetadata.id + ".meta"
    const patchMetadata = "INSERT {" + comment.metadata + "}"
    try {
      this.fu.patchFile(metadataLocation, patchMetadata)
    } catch (e) {
      console.error("Could not update metadata of paper with comment at location " + metadataLocation)
    }
  ...
  async getPaperCommentIds(paperMetadata: PaperMetadata) {
    ...
    if (!metadaLocationURI) {
      console.error("Paper does not have metadata location ", paperMetadata);
      return
    }
    const store = await this.getDataStoreFromFile(metadaLocationURI)
    return (await store.getQuads(null, SIOC + "reply_of", null, null)).map(quad => quad.subject.id || quad.subject.value)
  }

  async getCommentData(commentId: string) {
    const store = await this.getDataStoreFromFile(commentId)
    const commentQuads = await store.getQuads(commentId, null, null, null)
    let comment : Comment = {
      id: commentId
    };
  ...
  async getNotificationFromId(notificationId: string): Promise < Notification > {
    ...
    const commentQuad = await store.getQuads(notificationId, RDFS + "comment", null, null)[0]
    const comment = commentQuad && (commentQuad.object.id || commentQuad.object.value)

    const actorQuad = await store.getQuads(notificationId, AS + "actor", null, null)[0]
    const actor = actorQuad && (actorQuad.object.id || actorQuad.object.value)

    const objectQuad = await store.getQuads(notificationId, AS + "object", null, null)[0]
    const objectId = objectQuad && (objectQuad.object.id || objectQuad.object.value)

    const objectTypeQuad = await store.getQuads(objectId, RDF + "type", null, null)[0]
    const objectType = objectTypeQuad && (objectTypeQuad.object.id || objectTypeQuad.object.value)

    const objectReplyOfQuad = await store.getQuads(objectId, SIOC + 'reply_of', null, null)[0]
    const objectReplyOf = objectReplyOfQuad && (objectReplyOfQuad.object.id || objectReplyOfQuad.object.value)

    const objectCreatedAtQuad = await store.getQuads(objectId, SIOC + 'created_at', null, null)[0]
    const objectCreatedAt = objectCreatedAtQuad && (objectCreatedAtQuad.object.id || objectCreatedAtQuad.object.value)

    const objectCreatorQuad = await store.getQuads(objectId, SIOC + 'has_creator', null, null)[0]
    const objectCreator = objectCreatorQuad && (objectCreatorQuad.object.id || objectCreatorQuad.object.value)

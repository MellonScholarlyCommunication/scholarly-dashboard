...
export function InitializationView(props) {
  ...
  async function handleSubmit() {
    let thing = getThing(await getSolidDataset(webId), webId)
    ...
    if (!typeIndexUrl) alert('Please fill out all required information.')
          ...
          <CardContent>
            ...
            <Grid container spacing={2}>
              ...
              <Grid item xs={12} sm={9}>
                <Value autosave edit={true} dataType="url" property={AS.NAMESPACE + "outbox"}/>
              </Grid>

              <Grid item xs={12} sm={3}>
                <Typography gutterBottom variant="h5" component="h3">
                  type index file location
                </Typography>
              </Grid>
              <Grid item xs={12} sm={9}>
                <Value autosave edit={true} dataType="url" property={SOLID.publicTypeIndex} fullWidth/>
              </Grid>
              ...
              <Grid item xs={12} sm={9}>
                <Value autosave edit={true} dataType="url" property={ORCHESTRATORPREDICATE} />
              </Grid>
            </Grid>
            <Input type="submit" onClick={handleSubmit} value="Ready"/>
          </CardContent>
        </Card>
      </Container> }
...
//         new Blob([contents], { type: "text/turtle" }),
//         { contentType: "text/turtle", fetch: sesspropsion.fetch}
//       );
//     } catch (e) {
//       alert("Could not store index file in its requested container: " + e.message)
//     }
...
export async function checkRequiredProfileLinks(fetchFunction, webId) {
  ...
  console.log('initialized', links.orchestrator , links.outbox , links.publicTypeIndex)
...
export async function getProfileLinks (fetchFunction, webId) {
  ...
  links['orchestrator'] = getUrl(thing, ORCHESTRATORPREDICATE)
  return links
}